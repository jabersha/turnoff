	...
	public static void main(String[] args) {
		...
		for (int x = 0; x < s.length; x++) {
			...
			if (s[x][2].equals("frete")) {
				priorizados[iPrior] = s[x];	//	Atribui o conteudo a matriz de prioridade
		...
		for (int x = 0; x < iNPrior; x++) {	// Varre a matriz baseado no tamanho das incrementacoes
			for (int y = 0; y < naoPriorizados[x].length; y++) {//	Varre a matriz baseado no tamanho das colunas dos valores chumbados, pode colocar como 2/3 aki
		...
		for (int x = 0; x < iPrior; x++) {
			for (int y = 0; y < priorizados[x].length; y++) {
